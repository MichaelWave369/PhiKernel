...
"""
phik_shell_v0_1_1.py

Reference implementation for PhiKernel's fifth substrate module:
`phik-shell` (XiOS-facing CLI / terminal bridge).

Purpose
-------
This module gives the PhiKernel core braid an operable human interface.
It reads the runtime's Anchor, Capsule, Heart, and Coherence layers through a
single command surface.

What this version provides
--------------------------
- `phik status`      -> aggregate substrate status report
- `phik field`       -> coherence / TIEKAT field report
- `phik anchor show` -> signed anchor identity surface
- `phik capsule list`
- `phik capsule seal`
- `phik capsule restore <capsule_id>`
- `phik think`       -> context bundle for future orchestration / shell UX

Notes
-----
- This is a CLI-first implementation for v0.1.1.
- It integrates with the reference modules:
    - phik_anchor_v0_1_1.py
    - phik_capsule_v0_1_1.py
    - phik_heart_v0_1_1.py (reads persisted heart status)
    - phik_coherence_v0_1_1.py (reads persisted coherence frame)
- `think` is intentionally a local context-bundle bridge, not yet a live LLM
  orchestration layer.
"""
...
class PhiKernelShell:
    ...
    def cmd_field(self, args: argparse.Namespace) -> dict[str, Any]:
        frame = self._require_json(self.coherence_frame_file, "No coherence frame found. Run phik-heart with a coherence provider first.")
        return frame
    ...
    def cmd_capsule_seal(self, args: argparse.Namespace) -> dict[str, Any]:
        if not args.passphrase:
            raise ShellError("capsule seal requires --passphrase")
    ...
    def cmd_capsule_restore(self, args: argparse.Namespace) -> dict[str, Any]:
        if not args.passphrase:
            raise ShellError("capsule restore requires --passphrase")
    ...
    def cmd_think(self, args: argparse.Namespace) -> dict[str, Any]:
        """Build a shell-visible context bundle for later orchestration.

        This is intentionally local and deterministic in v0.1.1. It does not call
        any external LLM or agent directly.
        """
        anchor = self._safe_anchor_status()
        heart = self._read_json_optional(self.heart_status_file)
        field = self._read_json_optional(self.coherence_frame_file)
        ...
        bundle = {
            ...
            "generated_at": time.time(),
            "next_hint": self._next_hint(anchor=anchor, heart=heart, field=field, latest_capsule=latest_capsule),
        }
        return bundle

    def _build_parser(self) -> argparse.ArgumentParser:
        parser = argparse.ArgumentParser(prog="phik", description="PhiKernel XiOS shell")
        parser.add_argument("--runtime-root", default=str(DEFAULT_RUNTIME_ROOT), help="Base runtime root containing anchor/, capsule/, heart/, coherence/")
        ...
        parser.add_argument("--json", dest="json_output", action="store_true", help="Emit raw JSON instead of formatted terminal output")
        ...
        capsule_seal.add_argument("--capsule-type", default="working", choices=["canonical", "working", "checkpoint", "journal", "handoff"])
...
def resolve_runtime_paths(argv: list[str] | None = None) -> RuntimePaths:
    ...
    This lightweight pre-parse keeps path resolution centralized.
    """
